...
export default function TabVehiculos({ hogarId, userId, esAdmin }) {
          ...
          {cargandoItems ? (
            ...
            <>
              ...
              {itemsFiltrados.map(item => (
                <div key={item.id} className="bg-white border border-gray-100 rounded-2xl px-4 py-3 mb-2 card-ios" style={itemEditando?.id === item.id ? { borderLeft: '3px solid var(--color-vehiculos)' } : undefined}>
                  {renderItem(item)}
                </div>
              ))}
              {mostrarForm ? (
                ...
                <button onClick={() => { setMostrarForm(true); setFormData({}); setItemEditando(null); }} className="w-full py-3 mt-1 rounded-xl text-sm font-medium text-white hover:opacity-90 transition-opacity" style={{ backgroundColor: 'var(--color-vehiculos)' }}>
                  + Agregar
                </button>
              )}
            </>
  ...
  return (
    <div className="p-4">
      <div className="text-sm font-bold mb-3" style={{ color: 'var(--color-vehiculos)' }}>🚗 Vehículos</div>
      {cargando ? (
        <div className="text-center py-10 text-sm text-gray-400">Cargando...</div>
      ) : (
        <>
